(function(){
	...
	auxiliary.prototype = {
			nullBool : function(param){  // 对象为空判断 为空返回true，反之false
				...
				if( this.nullBool(param) || "" === param || 
						"undefined" === param || "null" === param || param.length === 0 )return true;
				return false;
 ...
 * @param func 回调函数
 */
var getInterfaceResult = function( AjaxUrl , AjaxData , func ) {
	var bool = new auxiliary() , resultData  = null;
	if(!bool.strBool(AjaxUrl)){
		if(bool.nullBool(AjaxData)) AjaxData = {};
		$.ajax({
...
window.onload = function(){
	document.onclick = function(){
		var w = window , e = w.event || arguments[0] , funcSource = null;
		funcSource = bAuxiliary.isIEBrowser() ? w.event.srcElement : e.target;
		displayRoute($(funcSource));
	}
}
function displayRoute(boolObj){
	if(null !== boolDisplayObj){
		var exception = boolDisplayObj["exception"];
		if(!bAuxiliary.arrBool(exception)){
			for(var i = 0 , j = exception.length ; i < j ; i++){
				var paramObj = $(exception[i]);
				if(isChildren( paramObj , boolObj)){
					return true;
				}
			}
		}
...
function isChildren(contrastObj , boolObj) { 
	var result = false;
	if(!bAuxiliary.nullBool(contrastObj)  && !bAuxiliary.nullBool(boolObj)){
		if("body" === boolObj[0].nodeName.toLowerCase()){
			result = false;
		}else{
			var boolResult = $(contrastObj).is(boolObj);
			if(boolResult){
				...
				result = isChildren(contrastObj , $(boolObj).parent());
			}
		}
	}
	return result;
}
function displayObj(boolObj) {
	
	function closeObj (closeObj){
		closeObj.close();
	}
	
	if(!bAuxiliary.arrBool(boolObj)){
		if("body" === boolObj[0].nodeName.toLowerCase()){
			var status = boolDisplayObj["status"]
			if(0 === status){
				// 第一次点击的是自己，故第一次不用关闭自己
				boolDisplayObj["status"] = (status + 1);
			} else {
				// alert("关闭该对象");
				closeObj(boolDisplayObj["handleObj"]);
			}
		}else{
			if(null === boolDisplayObj || undefined === boolDisplayObj){
				...
				var bObj = boolDisplayObj["boolObj"];
				var boolResult = $(bObj).is(boolObj);
				if(boolResult){
					// alert("子类 ， 不用处理");
					return;
				}else{
					displayObj($(boolObj).parent());
				}
			}
		}
	}
